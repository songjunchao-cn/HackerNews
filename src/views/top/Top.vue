...
export default {
  ...
  methods: {
    async getList () {
      let args = { query: `{stories(type: top) {id title time url by score descendants}}` }
      let res = await this.$api.getListApi(args)
      this.count = res.data.data.stories.length
      let params = { query: `{stories(type: top, first: 10, skip: ${this.currentPage} ) {id title time url by score descendants}}` }
